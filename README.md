...
When explicit rationale checkpoints are recorded, it can also write a separate internal review graph:
...
For a separate rationale graph, add explicit rationale checkpoints and render the CoT output:

```bash
python3 scripts/agent_thought_map.py checkpoint-cot --trace-id demo --kind decision --title "Chose adapter path" --summary "Adapters already normalize runtime events, so the change belongs in the renderer and CLI."
python3 scripts/agent_thought_map.py render-cot --trace-id demo
```
...
Each trace may contain both `events.jsonl` for the normal graph and `cot-events.jsonl` for explicit rationale checkpoints.
...
Create an explicit rationale checkpoint:

```bash
python3 scripts/agent_thought_map.py checkpoint-cot --trace-id demo --kind decision --title "Chose adapter path" --summary "Adapters already normalize runtime events, so the change belongs in the renderer and CLI."
```

Render the separate rationale graph:
...
## Rationale Graph Privacy

`latest-cot.md` is an internal rationale graph built from explicit `checkpoint-cot` events. It is not generated by reading hidden chain-of-thought or transcript files.
