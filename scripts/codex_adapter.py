#!/usr/bin/env python3
"""Thin Codex hook adapter for Agent Thought Map generic events."""
    ...
    for key in ("status", "outcome"):
        ...
        if isinstance(value, str) and value:
            ...
            if lower in {"blocked", "cancelled", "canceled"}:
                return "blocked"
            return "unknown"
    success = nested_get(payload, [("success",), ("tool_result", "success"), ("tool_response", "success")])
...
def infer_kind(hook_event: str, tool_name: str, target: str, status: str) -> str:
    ...
    if any(word in text for word in ("apply_patch", "edit", "write", "mv ", "cp ", "chmod")):
        return "action"
    if any(word in text for word in ("rg ", "grep", "sed ", "find ", "ls ", "cat ", "git status", "git diff", "pwd")):
...
def title_for_event(hook_event: str, kind: str, actor: str, target: str) -> str:
    ...
    return f"Observe {hook_event or 'Codex event'}"
...
def normalize_event(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("turn_id"):
        native_ref["turn_id"] = str(payload["turn_id"])
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Collect Codex hook events as generic Agent Thought Map events.")
    parser.add_argument("--data-dir", help="Trace data directory. Defaults to AGENT_THOUGHT_MAP_DATA, then PLUGIN_DATA.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    collect = subparsers.add_parser("collect", help="Read one Codex hook event from stdin and append a generic event.")
